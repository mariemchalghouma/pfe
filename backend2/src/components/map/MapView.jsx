...
const MapView = () => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            setIcons({
                depot: createPOIIcon('#3b82f6', '📦'),
                station: createPOIIcon('#6366f1', '⛽'),
                clientInterne: createPOIIcon('#f97316', '🏭'),
                clientExterne: createPOIIcon('#ef4444', '🏢'),
                arretConforme: createPOIIcon('#22c55e', '🅿'),
                arretNC: createPOIIcon('#ef4444', '🅿'),
            });
        }
    }, []);
    ...
    if (!icons) return <div className="h-full w-full flex items-center justify-center text-gray-400">Chargement de la carte...</div>;
        ...
        <div className="h-full w-full relative">
            ...
            >
                ...
                {layers.depots && depotsData.map(d => (
                    <Marker key={d.id} position={[d.lat, d.lng]} icon={icons.depot}>
                ...
                {layers.clients && clientsInternesData.map(c => (
                    <Marker key={c.id} position={[c.lat, c.lng]} icon={icons.clientInterne}>
                        <Popup>
                ...
                {layers.clients && clientsExternesData.map(c => (
                    <Marker key={c.id} position={[c.lat, c.lng]} icon={icons.clientExterne}>
                        <Popup>
                            <div className="text-sm">
                                <span className="font-semibold">{c.nom}</span><br />
                ...
                {layers.arrets && arretsData.map(a => (
                    <Marker key={a.id} position={[a.lat, a.lng]} icon={a.type === 'conforme' ? icons.arretConforme : icons.arretNC}>
                        <Popup>
                            <div className="text-sm">
