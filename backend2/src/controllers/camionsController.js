...
export const getCamionsTempsReel = async (date) => {
  try {
    ...
    const endExclusiveObj = new Date(`${targetDate}T00:00:00Z`);
    endExclusiveObj.setUTCDate(endExclusiveObj.getUTCDate() + 1);
    const endExclusiveDate = endExclusiveObj.toISOString().split("T")[0];
    ...
    const result = await pool.query(
      ...
      gps_7d AS (
        ...
        FROM local_histo_gps_all h
        WHERE h.camion IS NOT NULL
          AND h.gps_timestamp >= $1::date
          AND h.gps_timestamp < $2::date
      ),
...
export const getCamionsGantt = async (date) => {
  try {
    ...
    const voyageResult = await pool.query(
      ...
      FROM voyage_chauffeur
      WHERE "VOYDTD" >= $1::date
        AND "VOYDTD" < ($1::date + INTERVAL '1 day')
    ...
    const stopsResult = await pool.query(
      ...
      WHERE s.beginstoptime >= $1::date
        AND s.beginstoptime < ($1::date + INTERVAL '1 day')
    ...
    const ravitResult = await pool.query(
      ...
      WHERE COALESCE(date_trans, "date"::timestamp) >= $1::date
        AND COALESCE(date_trans, "date"::timestamp) < ($1::date + INTERVAL '1 day')
    ...
    const ouverturesResult = await pool.query(
      ...
      WHERE date_ouverture >= $1::date
        AND date_ouverture < ($1::date + INTERVAL '1 day')
...
export const getCamionTrajet = async (camion, date) => {
  try {
    ...
    if (date) {
      query +=
        " AND gps_timestamp >= $2::date AND gps_timestamp < ($2::date + INTERVAL '1 day')";
      params.push(date);
    }
