    ...
    try {
      ...
      Promise.allSettled(addressPromises).then((results) => {
        ...
        const newAddresses = new Map(
          resolved.map(({ plaque, address }) => [plaque, address]),
        );
        ...
        setMapData({
          markers: camionsData
            ...
            .map((camion) => {
              const config =
                statusConfig[camion.statut] || statusConfig.unknown;
              return {
                id: camion.plaque,
                lat: camion.lat,
                lng: camion.lng,
  ...
  return (
    <div className="flex h-full" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* ── Top bar ── */}
        <div
          className="flex items-center justify-between px-5 pt-4 pb-3 bg-white"
