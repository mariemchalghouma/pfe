...
const GestionPoi = () => {
  ...
  const handleSavePoi = async (poiData) => {
    try {
      ...
      alert(error?.message || "Erreur lors de l'enregistrement du POI.");
    }
  };
              ...
              <div className="flex justify-between items-center mb-10">
                <div>
                  ...
                  <p className="text-gray-500 text-[11px] mt-1">
                    Organisez vos points d&apos;intérêt par catégories
                    personnalisées
                  </p>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {groups.map((group) => {
                  ...
                  return (
                    <div
                      key={group.id}
                      className="group relative bg-white border border-gray-100 rounded-[32px] p-6 hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-500 overflow-hidden"
                    >
                      <div
                        className="absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 rounded-full opacity-[0.03] transition-transform group-hover:scale-150 duration-700"
                        style={{ backgroundColor: group.couleur }}
                      ></div>
                      <div className="flex justify-between items-start mb-6">
                        <div
                          className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner"
                          style={{ backgroundColor: `${group.couleur}15` }}
                        >
                          <div
                      ...
                      <div>
                        <h3 className="font-black text-gray-900 text-sm mb-1 tracking-tight">
                          {group.nom}
                        </h3>
                        <p className="text-gray-500 text-[10px] font-medium mb-6 line-clamp-1">
                          {group.description || "Pas de description définie"}
                        </p>
              ...
              {groups.length === 0 && (
                <div className="text-center py-20 bg-gray-50/50 rounded-[40px] border-2 border-dashed border-gray-200">
                  ...
                  <p className="text-gray-500 text-sm mb-8">
                    Commencez par créer votre premier groupe de POI
                  </p>
                  <button
            ...
            <div className="p-8 max-w-5xl mx-auto">
              <div className="flex flex-wrap items-center justify-between gap-6 mb-10 pb-8 border-b border-gray-100">
                <div>
                  ...
                  <p className="text-gray-500 text-[11px] mt-1">
                    Registre complet des modifications du système
                  </p>
                </div>
              ...
              {filteredHistory.length > 0 ? (
                  ...
                  {Object.entries(
                    ...
                    <div key={date} className="space-y-4">
                      ...
                      {items.map((item) => (
                        <div
                          key={item.id}
                          className="bg-white border border-gray-100 rounded-2xl p-5 hover:border-gray-200 hover:shadow-sm transition-all group overflow-hidden relative"
                        >
                          <div className="flex items-start gap-4">
                            <div
                              className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${item.action === "CREATE" ? "bg-green-50 text-green-600" : item.action === "UPDATE" ? "bg-blue-50 text-blue-600" : "bg-red-50 text-red-600"}`}
                            >
                              {item.action === "CREATE" ? (
                                <FiPlus />
                              ) : item.action === "UPDATE" ? (
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-start mb-3">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-tighter ${item.action === "CREATE" ? "bg-green-100 text-green-700" : item.action === "UPDATE" ? "bg-blue-100 text-blue-700" : "bg-red-100 text-red-700"}`}
                                >
                                  {item.action}
                                </span>
                              </div>
                                    ...
                                    .map((detail, dIdx) => {
                                      if (detail.includes(": ")) {
                                        ...
                                        return (
                                          <div
                                            key={dIdx}
                                            className="flex flex-col gap-1 py-2 px-3 bg-gray-50/50 rounded-xl border border-gray-50"
                                          >
                                            ...
                                            {newVal ? (
                                              <div className="flex items-center gap-2">
                                                <span className="text-[11px] font-medium text-gray-400 line-through decoration-gray-200/50">
                                                  {oldVal}
                                                </span>
                                      ...
                                      return (
                                        <p
                                          key={dIdx}
                                          className="text-[11px] font-medium text-gray-500 italic bg-gray-50/30 px-3 py-1.5 rounded-lg"
                                        >
                                          {detail}
                                        </p>
                                      );
                ...
                <div className="flex flex-col items-center justify-center py-32 text-gray-400 bg-gray-50/50 rounded-[40px] border-2 border-dashed border-gray-100">
                  ...
                  <p className="text-[11px] text-gray-500 mt-1 max-w-xs text-center">
                    Aucune modification ne correspond à vos filtres de
                    recherche.
                  </p>
                </div>
              )}
